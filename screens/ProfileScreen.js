...
const FAVES_KEY = '@inklore_favorites';
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    AsyncStorage.multiGet(['@inklore_avatar', '@inklore_banner', FAVES_KEY]).then(([[, av], [, bn], [, fv]]) => {
  ...
  async function pickAvatar() {
    ...
    if (!result.canceled) {
      ...
      if (url) {
        await AsyncStorage.setItem('@inklore_avatar', url);
        setAvatarUri(url);
      }
    }
  }
  ...
  async function pickBanner() {
    ...
    if (!result.canceled) {
      ...
      if (url) {
        await AsyncStorage.setItem('@inklore_banner', url);
        setBannerUri(url);
      }
    }
  }

...
export default function ForYouScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      AsyncStorage.multiGet([AI_REC_KEY, '@inklore_genre_prefs', AGE_VERIFIED_KEY, NSFW_KEY]).then(
            ...
            supabase.from('user_genre_preferences')
              ...
              .then(({ data }) => {
                if (data?.length) {
                  ...
                  AsyncStorage.setItem('@inklore_genre_prefs', JSON.stringify(weights)).catch(() => {});
  ...
  function onRefresh() {
    ...
    AsyncStorage.multiGet([AI_REC_KEY, '@inklore_genre_prefs']).then(
      ([[, aiRaw], [, genreRaw]]) => {
        const aiOn = aiRaw === null ? true : aiRaw === 'true';
        setAiRecEnabled(aiOn);
        if (aiOn && genreRaw) {
    ...
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      <ScrollView
        ...
        {creatorSeries.length > 0 && (
          <>
            <SectionTitle colors={colors} delay={160} focusKey={focusKey}>✨ From Inklore Creators</SectionTitle>
            <FlatList
              horizontal
              data={creatorSeries}
